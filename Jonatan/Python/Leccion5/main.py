...
# 7.1
def listarTerminos(**terminos):  # losmas utilizado es **kwargs para recibir los argumentos
    for llave, valor in terminos.items():  # .items para recorrer diccionarios, *kwargs significa: key word argument
        print(f'{llave} : {valor}')


listarTerminos()  # no recibe nada, nada se va a mostrar
listarTerminos(IDE='Integrated Develoment Enviroment', PK='Primary Key')  # agregando elemento a un diccionario


def desplegarNombres(nombres):
    for nombre in nombres:  # nombres es un iterador
        print(nombre)


nombres2 = ['Tito', 'Pedro', 'Carlos']
desplegarNombres(nombres2)
desplegarNombres('Carla')  # itera letra por letra
# desplegarNombres(10) no es un objeto iterable
desplegarNombres((10, 11))  # lo convertimos a una tupla, en un solo elemento no olvidar la coma (10,)
desplegarNombres([22, 55])  # lo convertimos en una lista

# 7.4 Funciones Recursivas
#recursividad: una funcion recursiva se manda a llamra asi misma para completar una sierta tarea
